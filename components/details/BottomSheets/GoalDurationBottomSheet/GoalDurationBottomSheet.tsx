  ...
  return (
    <>
      <Drawer.Root open={openBottomSheet}>
        <Drawer.Portal>
          <Drawer.Overlay className="w-[100vw] h-full bg-neutral-700/50" />
          <Drawer.Content className="w-[360px] h-[585px] relative bg-background-alternative rounded-tl-lg rounded-tr-lg shadow-[0px_0px_4px_0px_rgba(0,0,0,0.24)] overflow-hidden">
            <div>
              <header>
                <p>목표 기간 수정</p>
                <button
              ...
              <button
                onClick={() => {
                  onEdit({
                    type: durationType,
                    value:
                      durationType === "month" ? selectedMonth : selectedDate,
                  });
                }}
                type="button"
                data-state="enabled"
